"""Read/write the ``.base.yaml`` member of a packed (``.tgz``) archive.

Sits in ``core`` so both ``archive`` and ``metadata`` can use it
without violating the layering rules. Stdlib + PyYAML only."""
